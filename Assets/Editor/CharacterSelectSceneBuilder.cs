...
public static class CharacterSelectSceneBuilder
{
	...
	public static void Build()
	{
		...
		// ── 주사위 세부 규칙 팝업 ─────────────────────────────────────────
		var rulesDimmer = CreateDimmer(canvasObject.transform, "RulesDimmer");
		...
		var diceExplImporter = AssetImporter.GetAtPath(diceExplSpritePath) as TextureImporter;
		if (diceExplImporter != null)
		{
			bool reimport = false;
			if (diceExplImporter.textureType != TextureImporterType.Sprite ||
				diceExplImporter.spriteImportMode != SpriteImportMode.Single)
			{
				diceExplImporter.textureType = TextureImporterType.Sprite;
				diceExplImporter.spriteImportMode = SpriteImportMode.Single;
				reimport = true;
			}
			var sts = new TextureImporterSettings();
			diceExplImporter.ReadTextureSettings(sts);
			if (sts.spriteMeshType != SpriteMeshType.Tight)
			{
				sts.spriteMeshType = SpriteMeshType.Tight;
				diceExplImporter.SetTextureSettings(sts);
				reimport = true;
			}
			if (reimport)
				diceExplImporter.SaveAndReimport();
		}
	...
	private static DiceRulesAreaResult BuildDiceRulesArea(Transform canvasParent)
	{
		...
		// 배경
		var bg = CreateUIPanel(area.transform, "RulesBg", new Color(0.08f, 0.10f, 0.20f, 0.85f));
		SetStretch(bg);

		// 제목
		var title = CreateTMPText(area.transform, "RulesTitle",
			"주사위 전투 규칙", 26, FontStyles.Bold,
			new Color(0.95f, 0.95f, 1f, 1f));
		...
		var body = CreateTMPText(area.transform, "RulesBody",
			string.Join("\n", bodyLines), 20, FontStyles.Normal,
			new Color(0.82f, 0.85f, 1f, 0.95f));
	...
	{
		var popup = CreateUIPanel(canvasParent, "DiceRulesPopup",
			new Color(0.08f, 0.09f, 0.18f, 0.97f));
		CenterPopup(popup, 620, 480);
		popup.AddComponent<SimplePopup>();

		// 제목
		var title = CreateTMPText(popup.transform, "Title", "주사위 족보 & 데미지",
		...
		var body = CreateTMPText(popup.transform, "DetailBody",
			string.Join("\n", detailLines), 20, FontStyles.Normal,
			new Color(0.88f, 0.90f, 1f, 1f));
		var bodyRt = body.GetComponent<RectTransform>();
		...
		var closeLabelObj = CreateTMPText(closeBtn.transform, "Label", "✕",
			32, FontStyles.Bold, new Color(0.7f, 0.7f, 0.7f, 1f));
		SetStretch(closeLabelObj);
		closeLabelObj.GetComponent<TMP_Text>().alignment = TextAlignmentOptions.Center;

		// 호버 시 빨간색 전환
		var hoverEffect = closeBtn.AddComponent<UIHoverEffect>();
		SetPrivateField(hoverEffect, "targetText", closeLabelObj.GetComponent<TMP_Text>());
	...
	private static void AddSceneToBuildSettings(string path)
		=> SceneBuilderUtility.AddSceneToBuildSettings(path);
}
